window.Stage = function Stage(aCanvas){
	...
	function quantumYarrow(){
		...
		return ({ 
							...
							endColor: startColor //colorPalette[parseInt(Math.random()*paletteSize)]
						})
	}
	...
	this.update = function(){
		...
		for(var i=0; i<nSticks;i++){
			...
			*/
			...
			var theta = Math.atan2(pair.y-p.y,pair.x-p.x);

			p.dx = Math.cos(theta)*0.2+p.dx*0.8; // gradual turn towards the heading
			p.dy = Math.sin(theta)*0.2+p.dy*0.8;
