window.Stage = function Stage(aCanvas){
	...
	let grid = [];
	let seed = [ parseInt(Math.random()*256), parseInt(Math.random()*256), parseInt(Math.random()*256) ];
	...
	function randomPlot(row,col){
		...
		// don't wrap around
		let seed = [
			(gaussianRandom(stage.variance/256)-0.5)*128 + value[0],
			(gaussianRandom(stage.variance/256)-0.5)*128 + value[1],
			(gaussianRandom(stage.variance/256)-0.5)*128 + value[2]
		];

		for(let i=0;i<3;i++){	
			seed[i] = (seed[i] < 0) ? Math.abs(seed[i]) : seed[i];
			seed[i] = (seed[i] > 255) ? 255-seed[i] : seed[i];
		}
	...
	this.clear = function(){
		...
		for(let k=stage.seeds;k--;){
			...
			grid[i][j] = [ Math.random()*360, Math.random()*360, Math.random()*360 ];
			stage.filledCells++;
			randomPlot(i,j);
		}
